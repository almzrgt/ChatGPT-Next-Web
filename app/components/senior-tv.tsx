...
type AppPage = "home" | "remote" | "care" | "settings";
...
export function SeniorTvApp() {
  ...
  const [careEnabled, setCareEnabled] = useState(true);
  const [remoteNotice, setRemoteNotice] = useState("电视正在播放：央视一套");
  ...
  const pages: Array<{ id: AppPage; label: string; icon: IconProps["name"] }> =
    [
      { id: "home", label: "首页", icon: "home" },
      { id: "remote", label: "遥控器", icon: "tv" },
      { id: "care", label: "健康关怀", icon: "heart" },
  ...
  return (
    <main className={styles.appShell}>
      ...
      <div className={styles.pageContent}>
        ...
        {activePage === "remote" && (
          <section className={styles.toolPage} aria-labelledby="remote-title">
            <div className={styles.pageHeading}>
              <span className={styles.pageIcon}>📺</span>
              <div>
                <small>客厅电视 · 已连接</small>
                <h1 id="remote-title">电视遥控器</h1>
                <p>{remoteNotice}</p>
              </div>
            </div>
            <div className={styles.remoteCard}>
              <div className={styles.quickControls}>
                <button
                  onClick={() => setRemoteNotice("电视已关机，再按一次可开机")}
                >
                  <span>⏻</span>电视开关
                </button>
                <button onClick={() => setRemoteNotice("已返回电视首页")}>
                  <Icon name="home" />
                  电视首页
                </button>
              </div>
              <div className={styles.remoteBody}>
                ...
                <div className={styles.directionPad}>
                  <button aria-label="向上">▲</button>
                  <button aria-label="向左">◀</button>
                  <button
                    className={styles.okButton}
                    onClick={() => setRemoteNotice("已确认您的选择")}
                  >
                    确定
                  </button>
                  <button aria-label="向右">▶</button>
                  <button aria-label="向下">▼</button>
                </div>
                <div className={styles.adjustGroup}>
                  <strong>频道</strong>
                  <button onClick={() => setRemoteNotice("已切换到上一个频道")}>
                    ＋
                  </button>
                  <span>台</span>
                  <button onClick={() => setRemoteNotice("已切换到下一个频道")}>
                    －
                  </button>
                </div>
              </div>
              <button
                ...
                onClick={() => setRemoteNotice("已返回上一级")}
              >
                ↩ 返回上一级
              </button>
            </div>
        ...
        {activePage === "care" && (
          <section className={styles.toolPage} aria-labelledby="care-title">
            <div className={styles.pageHeading}>
              <span className={`${styles.pageIcon} ${styles.heartIcon}`}>
                <Icon name="heart" />
              </span>
              <div>
                <small>安心陪伴每一天</small>
                <h1 id="care-title">健康关怀</h1>
                <p>{careEnabled ? "关怀守护正在工作" : "关怀守护目前已暂停"}</p>
              </div>
            </div>
            <div className={styles.careGrid}>
              <article className={styles.statusCard}>
                ...
                <p>
                  {careEnabled
                    ? `${name} 会留意您的声音和状态，发现异常会先询问您。`
                    : "开启后，小乐可以在获得授权后留意您的状态。"}
                </p>
                <button
                  className={careEnabled ? styles.toggleOn : ""}
                  onClick={() => setCareEnabled(!careEnabled)}
                  aria-pressed={careEnabled}
                >
                  <i />
                  {careEnabled ? "已开启" : "已关闭"}
                </button>
              ...
              <article className={styles.statusCard}>
                <div>
                  <span className={styles.familyAvatar}>女</span>
                  <strong>紧急联系人</strong>
                </div>
                <p>
                  女儿 · 王女士
                ...
                <button onClick={() => window.alert("正在联系王女士（演示）")}>
                  ☎ 一键联系家人
                </button>
              </article>
              <article className={styles.statusCard}>
            ...
            <div className={styles.safetyTip}>
              <strong>温馨提示</strong>
              <p>
                健康关怀不能代替医生。如有明显不适，请及时联系家人或拨打急救电话。
              </p>
            </div>
          </section>
        )}
        ...
        {activePage === "settings" && (
          <section className={styles.toolPage} aria-labelledby="settings-title">
            <div className={styles.pageHeading}>
              <span className={styles.pageIcon}>
                <Icon name="settings" />
              </span>
              <div>
                <small>按您的习惯来调整</small>
            ...
            <div className={styles.settingsList}>
              ...
              <div className={styles.settingRow}>
                <div>
                  <strong>伙伴声音</strong>
                  <small>选择更容易听清的声音</small>
                </div>
                <div className={styles.settingChoices}>
                  {["温柔亲切", "清晰响亮"].map((item) => (
                    <button
                      key={item}
                      className={voice === item ? styles.choiceActive : ""}
                      onClick={() => setVoice(item)}
                    >
                      {item}
                    </button>
              ...
              <div className={styles.settingRow}>
                <div>
                  <strong>唤醒暗号</strong>
                  <small>除了喊名字，也可以用约定动作</small>
                </div>
                <span className={styles.codePill}>咳嗽一声</span>
              </div>
              <div className={styles.settingRow}>
                <div>
                  <strong>电视连接</strong>
      ...
      <nav className={styles.nav} aria-label="主要页面">
        {pages.map((page) => (
          <button
            key={page.id}
